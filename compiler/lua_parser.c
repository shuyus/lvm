...
static void singlevar(FuncState* fs, expdesc* e, TString* n) {
	...
	if (e->k == VVOID) { // variable is in global
		...
		lua_assert(e->k == VUPVAL); /* 这时还没有从全局表中找，info为0 */
		codestring(fs, n, &k); 
		luaK_indexed(fs, e, &k);
	}
}
...
static int explist(FuncState* fs, expdesc* e) {
	int var = 1;
	expr(fs, e); /* 这时至少有一个参数，先分析第一个参数表达式 */
...
static void suffixdexp(struct lua_State* L, LexState* ls, FuncState* fs, expdesc* e) {
	...
	switch (ls->t.token) {
	case '(': { /* 函数调用 */
		luaX_next(L, ls);/*第一个参数或)读进来了 */

		luaK_exp2nextreg(fs, e); /* 第一个TK_NAME的e生成字节码 */
		funcargs(fs, e);
	} break;
	case ',': break;
	default: 
...
LClosure* luaY_parser(struct lua_State* L, Zio* zio, MBuffer* buffer, Dyndata* dyd, const char* name) {
	...
	ls.h = luaH_new(L);
	setgcovalue(L->top, obj2gco(ls.h));
	increase_top(L);
